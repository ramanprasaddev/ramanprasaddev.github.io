window.addEventListener('DOMContentLoaded', async function () {
    ...
    document.querySelectorAll('.githubCard').forEach(async function (el) {
      ...
      el.innerHTML = `
      <div data-id="${data.full_name || 'NOT_FOUND'}" onclick="window.open('https://github.com/${data.full_name || elevenvac}?utm_source=ramanprasaddev.github.io')" class="card rounded-custom h-full">
  ...
  /*
      <div data-id="${data.full_name || 'NOT_FOUND'}" onclick="window.open('https://github.com/${data.full_name || raman}')" class="oal-comps-xl-card rounded-cxl mt-3">
          <p class="section-regular-text-b oal-comps-xl-card-link">raman/</a>${data.full_name || "No data"}</p>
